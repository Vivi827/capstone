...
- NICT는 567행 중 208행이 Formality/Energy에서만 다름 (평균 |Δ| F 2.57 / E 1.10).
  **정정 (cycle 4d, 2026-09-12)**: 이 차이는 "사람이 수정한 라벨"이 아니다.
  `nict_jle_labeled_human_600.jsonl`을 `nict_jle_labeled_draft_600.jsonl`과
  직접 대조한 결과 600/600 완전히 동일하고, `labeler` 필드는 전부
  `codex_accept_draft_per_user`(초안 자동 수용) — **독립적인 사람 편집 0건.**
  208행 차이는 "현재 코드의 `draft_axes()` 출력"과 "그 당시 저장된 초안 라벨"이
  다른 것뿐 (규칙/코드가 그 사이에 바뀜). **즉 3,137행 실험 학습셋 전체에
  독립적인 사람 검증 라벨은 하나도 없다.** 이 프로젝트에서 독립 사람 라벨은
  gold-200(dev)과 예약 176(test)뿐이다.
...
## 12. B0-B4 실험: 선형(능선회귀) 대안 모델 (2026-09-11~12, cycle 4c/4d로 정정됨)

> 용어 정정(cycle 4d): 이전 버전에서 "로지스틱 회귀"라고 잘못 표기했다.
> B3는 로지스틱이 아니라 **능선회귀(ridge regression, L2 정규화 선형회귀)**다.
...
- **B1 — 기존 약한 라벨로 학습한 최근접 이웃 회귀.** `TfidfKnnAxisRegressor`
  ...
  확인됨); NICT도 **독립적인 사람 검증 라벨 0건**(cycle 4d 정정, 아래 참조) —
  600/600 "human_reviewed" 행이 draft와 byte-identical, `labeler=codex_accept_draft_per_user`.
...
10. **축별 숨은 퇴행 (cycle 4d에서 수정 후 수치로 갱신)**: Energy rho는
    rule(0.505) > hybrid(0.426) > B3b(**0.344**, 수정 전 보고치 0.389는 오기) —
    hybrid 대비 −0.082로 **계약서 §10의 축별 0.03 허용폭을 점추정 기준 초과**.
    Energy는 규칙이 최고. AMI MAE도 rule(11.76)이 모든 ML 계열보다 낮다
    (B3b **12.47**, 수정 전 보고치 12.64는 오기). B4도 전축 지배는 아님 —
    Humor 풀링 rho는 B3b가 B4보다 나음(0.236>0.201)이지만 **Intimacy는 수정 후
    B4가 B3b보다 낫다(0.359>0.337, 수정 전 보고는 반대로 틀렸음)**. Curiosity는
    B3a가 B4보다 낫다. **"B3b/B4가 이긴다"는 단일 문장은 성립하지 않는다.**

### cycle 4d 추가 정정 (2026-09-12, 코덱스 Round 2 + 직접 재검증)

11. **최우선 정정 — "NICT 208행 사람 수정" 주장은 틀렸다.**
    `nict_jle_labeled_human_600.jsonl`을 `nict_jle_labeled_draft_600.jsonl`과
    직접 대조: **600/600 byte-identical**, `labeler` 전부
    `codex_accept_draft_per_user`(초안 자동 수용) — **독립적인 사람 편집 0건.**
    208행 차이는 "현재 `draft_axes()` 코드 출력"과 "그때 저장된 초안"의 코드
    드리프트일 뿐이다. **정정: 3,137행 실험 학습셋 어디에도 독립적인 사람 검증
    라벨이 없다.** 이 프로젝트의 독립 사람 라벨은 gold-200(dev)과 예약 176
    (test) 두 곳뿐이다. 이는 Formality 편향 보정 시 "NICT 사람 라벨로 보정"이
    불가능함을 뜻한다(아래 14).

12. **ridge 수정 후 부트스트랩 재계산**: `scripts/bootstrap_b0_b4_comparison.py`
    (커밋 `6c322ce`)로 재계산, 코덱스가 재현 확인:

    | 짝지은 평균 rho 차이 | 관측값 | 95% 구간 |
    |---|---:|---|
    | B3a(수정 후) − B2 | +0.020 | [-0.021, +0.068] — **0 포함** |
    | B3b(수정 후) − B2 | +0.054 | [-0.016, +0.132] — **0 포함** |
    | B4 − B3b(수정 후) | +0.080 | [+0.030, +0.134] — 명확히 양수 |

    **ridge 후보 둘 다 hybrid 대비 95% 신뢰수준에서 구별 안 됨.** Flash-Lite
    직접 호출만 그 앞 단계보다 확실히 낫다. 이 구간은 반복 dev 분석·설정
    선택·비-blind 재검수·rubric 불확실성을 반영 못한 탐색적 수치다.

13. **어휘 통일 비교 (모델 계열 효과 격리)**: ridge를 kNN과 완전히 같은 어휘
    17,913개로 맞춰도(`min_df=1, max_features=999999`) **rho 0.420** — kNN
    0.362보다 여전히 높음. 코덱스가 두 구현의 IDF 사전이 완전히 동일하고
    2,940행 전체에서 feature 값 차이가 0임을 확인 — 특징 쪽 교란은 없다.
    단, Humor는 matched-ridge(0.075)가 kNN(0.119)보다 낮음 — 전축 지배 아님.

14. **k/ngram 선택편향 정량화 (nested holdout)**: purged 학습셋 내부에서
    80/20 그룹 분할(seed `inner-nested-selection-20260912`, inner-train 2,305
    / inner-holdout 635) → 15개 설정을 inner-holdout(약한 라벨 기준)에서만
    비교 → 승자는 k=15/trigram(inner rho 0.492) → 그 하나만 전체 purged로
    재학습 → dev-200에서 **1회만** 평가: **rho 0.354**. dev에서 직접 고른
    최댓값(0.362, k15/bigram)과 차이는 작음(~0.01) — 이번 경우 선택편향은
    크지 않았다. 보고할 때는 0.354(정직한 추정)와 0.362(실제 B2/B3에 쓰인,
    약간 낙관적인 값)를 함께 표기한다.

15. **Formality 편향 보정 방법 — 코덱스 권고**: 출처별 B3b Formality 편향은
    AMI +17.2 / NICT +18.6 / Taskmaster +9.9로 **균일한 상수가 아니다** — 단순
    오프셋 하나로 보정해도 충분하다는 보장이 없다. 검토한 선택지:
    (a) 보정 안 하고 결함으로 보고, 예약 176 판정 때 하나의 확정 후보로
    받아들이거나 기각 — **기본값, 이번 사이클 채택.**
    (b) 이미 채점된 학습 데이터로 보정 — **불가능**: §11 정정으로 독립 사람
    기준점이 없음이 확인됨(NICT "사람" 라벨=draft 그대로).
    (c) dev-200을 보정용/보고용으로 쪼갬 — 탐색적으로는 가능하나 두 조각 다
    이미 진단·모델 선택에 쓰여서 깨끗한 holdout 아님, dev를 더 소모함.
    (d) **새로운 소규모 사람 라벨링** — 예약·dev와 안 겹치는, 이미 교사가
    채점한 학습 그룹 중 일부를 출처 균형 있게 뽑아 Formality만 blind 채점,
    학생 학습에서는 제외, 학습에는 안 씀. **유일하게 진짜 독립 기준점을
    만드는 방법이지만 새 사람 라벨링이 필요 — 목표1(최소화)과 상충하는
    사용자 결정 사항.**

16. **B3b 배포 어댑터: 보류 (코덱스 권고)**: 부트스트랩만으로 ridge를
    기각하는 건 아니다(계약 gate는 평균 rho 95% 우위만 요구하지 않음, MAE
    비열등·축별 허용폭·독립 재현도 있음). 하지만 Energy 퇴행(−0.082, 허용폭
    초과)과 미해결 Formality 결함이 있는 채로 배포 어댑터를 만드는 건 아직
    이르다. 어댑터 작업은 Formality 방침 결정 + Energy 퇴행 설명/해결 +
    축·출처별 표 완비 이후로 미룬다.
...
| B1 kNN·기존 약한 라벨 (k15/bigram, dev-선택 최댓값) | 0.362 | 13.21 | nested holdout 정직한 추정은 0.354 (k15/trigram) |
| B2 hybrid | 0.408 | 12.36 | |
| B3a ridge·같은 약한 라벨 (어휘 다름, 4천) | 0.428 | 12.67 | 어휘 통일(1.8만) 시 0.420 — 모델 효과는 유지 |
| B3b ridge·Flash-Lite 라벨 | 0.462 | 11.94 | hybrid 대비 95% CI [-0.016,+0.132] 0 포함; **Formality MAE 15.6(hybrid 9.6), Energy rho 0.344(hybrid 0.426, 허용폭 초과)** |
| B4 Flash-Lite 직접 호출 | 0.542 | 11.00 | 전축 지배 아님(Curiosity는 B3a가 나음); 턴당 API 비용; 병렬화는 막혀있지 않으나 미구현·미실측 |

### 이번 사이클 결론 (cycle 4d 반영)

**아직 어느 팔도 프로덕션 결정을 내릴 수 있는 상태가 아니다.** B3b는 점추정
상 hybrid보다 낫지만 (1) 그 우위가 통계적으로 확실하지 않음(95% CI가 0 포함)
(2) Formality 편향 미해결, 보정할 독립 기준점도 없음 (3) Energy가 허용폭
넘게 퇴행 (4) 배포 어댑터 없음. **학습셋 전체(3,137행)에 독립 사람 라벨이
전혀 없다는 것도 이번에 확인됨** — 이는 "교사 재현" 목표로 U1을 바꾸는 안을
더 약하게 만든다(재현할 "교사"조차 사람 기준이 없으므로).
